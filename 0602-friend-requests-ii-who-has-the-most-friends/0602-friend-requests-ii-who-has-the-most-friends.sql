...
with A as (select requester_id as id, count(accepter_id) as c1 from RequestAccepted group by requester_id 
union all
select accepter_id as id, count(requester_id) as c1 from RequestAccepted group by accepter_id)

select A.id, sum(A.c1) as num from A group by A.id order by num desc limit 1
